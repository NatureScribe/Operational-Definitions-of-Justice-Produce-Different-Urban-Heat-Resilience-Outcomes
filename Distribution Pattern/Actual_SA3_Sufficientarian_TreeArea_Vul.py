...
for hvi in range(45):
    ...
    for i in range(sufficientarian_expected_tree_area.shape[0]):
        if (15-hvi)/3-0.1 < integrated_dataset[i, 10] and integrated_dataset[i, 3] / integrated_dataset[i, 1] < integrated_dataset[44-hvi, 15] and sufficientarian_percentage[i, 0] == 1:
            under_threshold += 1
    ...
    while count < under_threshold:
        ...
        if more_trees_sqm < 1 or next_min_value > integrated_dataset[44-hvi, 15]:
            break
...
filepath_pred = 'Actual_SA3_Sufficientarian_TreeArea_Vul.xlsx'
