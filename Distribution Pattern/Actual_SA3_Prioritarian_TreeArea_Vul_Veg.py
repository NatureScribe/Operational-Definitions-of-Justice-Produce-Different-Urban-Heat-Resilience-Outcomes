...
integrated_dataset = pd.read_excel("SA3_Integrated_Dataset_Filtered_Vul_Veg.xlsx")
...
for i in range(integrated_dataset.shape[0]):
    prio_weights += integrated_dataset[i, 1] * integrated_dataset[i, 11]
print(prio_weights)

for i in range(prioritarian_expected_tree_area.shape[0]):
    prioritarian_expected_tree_area[i, 0] = integrated_dataset[i, 3] + more_trees_sqm * integrated_dataset[i, 1] * integrated_dataset[i, 11] / prio_weights
...
while additional_trees != 0:
    ...
    for i in range(prioritarian_expected_tree_area.shape[0]):
        if i not in List_limited_veg_area:
            prioritarian_expected_tree_area[i, 0] += additional_trees * integrated_dataset[i, 1] * integrated_dataset[i, 11] / prio_weights


df_pred = pd.DataFrame(prioritarian_expected_tree_area)
filepath_pred = 'Actual_SA3_Prioritarian_TreeArea_Vul_Veg.xlsx'
