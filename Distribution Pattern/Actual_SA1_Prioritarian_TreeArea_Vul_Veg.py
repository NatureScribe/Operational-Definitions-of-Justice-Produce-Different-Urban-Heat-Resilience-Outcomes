...
while additional_trees != 0:
    ...
    for i in range(prioritarian_expected_tree_area.shape[0]):
        if i not in List_limited_veg_area:
            prioritarian_expected_tree_area[i, 0] += additional_trees * integrated_dataset[i, 1] * integrated_dataset[i, 10] / prio_weights


df_pred = pd.DataFrame(prioritarian_expected_tree_area)
filepath_pred = 'Actual_SA1_Prioritarian_TreeArea_Vul_Veg.xlsx'
