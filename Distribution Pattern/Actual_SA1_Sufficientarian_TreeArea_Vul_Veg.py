...
for hvi in range(12):
    ...
    for i in range(sufficientarian_expected_tree_area.shape[0]):
        if (15-hvi)/3-0.1 < integrated_dataset[i, 11] and integrated_dataset[i, 3] / integrated_dataset[i, 1] < integrated_dataset[11-hvi, 16] and sufficientarian_percentage[i, 0] == 1:
            under_threshold += 1
    ...
    while count < under_threshold:
        ...
        if more_trees_sqm < 1 or next_min_value > integrated_dataset[11-hvi, 16]:
            break
...
filepath_pred = 'Actual_SA1_Sufficientarian_TreeArea_Vul_Veg2.xlsx'
