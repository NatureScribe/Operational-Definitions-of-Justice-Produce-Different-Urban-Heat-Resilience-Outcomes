...
for i in range(sufficientarian_expected_tree_area.shape[0]):
    sufficientarian_percentage[i, 0] = integrated_dataset[i, 42] / integrated_dataset[i, 3]
count = 0
while count < integrated_dataset.shape[0]:
    ...
    for i in range(len(min_indices)):
        allocation = (next_min_value - min_value) * integrated_dataset[min_indices[i], 3]
        if allocation > more_trees_sqm:
            sufficientarian_percentage[min_indices[i], 0] = more_trees_sqm / integrated_dataset[min_indices[i], 3] + min_value
            break
...
for i in range(sufficientarian_expected_tree_area.shape[0]):
    sufficientarian_expected_tree_area[i, 0] = integrated_dataset[i, 3] * sufficientarian_percentage[i, 0]
...
while additional_trees != 0:
    ...
    prio_weights = 0
    for i in range(sufficientarian_expected_tree_area.shape[0]):
        if sufficientarian_expected_tree_area[i, 0] > integrated_dataset[i, 43]:
            List_limited_veg_area.append(i)
            additional_trees += sufficientarian_expected_tree_area[i, 0] - integrated_dataset[i, 43]
            sufficientarian_expected_tree_area[i, 0] = integrated_dataset[i, 43]
    ...
    for i in range(sufficientarian_expected_tree_area.shape[0]):
        if i not in List_limited_veg_area:
            sufficientarian_percentage[i, 0] = sufficientarian_expected_tree_area[i, 0] / integrated_dataset[i, 3]
        else:
...
filepath_pred = 'Actual_MB_Sufficientarian_TreeArea_Veg.xlsx'
